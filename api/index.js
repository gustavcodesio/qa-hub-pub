...
// shared/app-icon-manifest.ts
var BUNDLED_APP_ICONS = {
...
function bundledAppIconUrl(appId) {
...
function resolveServedIconPath(appId, folder) {
  const bundled = bundledAppIconUrl(appId);
  if (bundled) {
    const fromPublic = path4.join(rootDir, "public", bundled.replace(/^\//, ""));
    if (existsSync4(fromPublic)) return fromPublic;
  }
  return resolveAppIconPath(workspaceRoot2, folder);
}
app.get("/api/apps", (_req, res) => {
  const apps = store.listApps().map((item) => ({
    ...item,
    iconUrl: bundledAppIconUrl(item.id) ?? (resolveAppIconPath(workspaceRoot2, item.folder) ? appIconPublicUrl(item.id) : null)
  }));
  res.json({ apps });
});
